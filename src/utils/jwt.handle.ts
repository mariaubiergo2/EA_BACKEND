...
const generateTokenCompleted = (idUser: string, name: string, surname: string, username: string, role:string, level:Number) => {
  const jwt = sign({ idUser, name, surname, username, role, level }, JWT_SECRET, {
    expiresIn: "2h",
  });
  return jwt;
};
